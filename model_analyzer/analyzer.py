...
def run(model_file: str, analyzers=[]):
    ...
    if "variable" in analyzers:
        variable_analyzer.process_variable_types(
            m, data, objective_variables, constraint_variables
        )
...
def process_model_type(m, data):
    ...
    part4_short = (
        "LP" if part1_short == "" and part2_short == "" and part3_short == "" else "P"
    )
    part4_long = (
        "Linear Program"
        if part1_long == "" and part2_long == "" and part3_long == ""
        else "Program"
    )
