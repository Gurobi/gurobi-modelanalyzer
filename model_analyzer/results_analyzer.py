...
def kappa_explain(
    ...
    if expltype == "COLS":
        ...
        print("Invalid specification for expltype:", str(expltype) + ": exiting.")
        return None
    ...
    if relobjtype == "QP":
        ...
        print(
            "Consider setting method to LASSO or submatrix parameter",
            "to False to obtain smaller explanation.",
        )
        return None
    else:
        print("Invalid specification for relobjtype:", str(relobjtype) + "; exiting.")
        return None
    ...
    explmodel.feasRelax(
        relobjtype, minrelax, None, None, None, excons[0:nbas], [1.0] * nbas
    )
    #
    ...
    if expltype == BYROWS:
        ...
        for yv in yvars:
            ...
            if abs(yval) <= zerotol:
                ...
                thiscon = rconsdict[yv.index]
                explname = "(mult=" + str(yval) + ")" + thiscon.ConstrName
        ...
        # else:
        #    combinedsense = GRB.EQUAL
        resmodel.addLConstr(combinedlhs, combinedsense, combinedrhs, COMBINEDROW)
...
def extract_basis(
    ...
    build_explmodel(model, explmodel, modvars, modcons, RSinginfo, CSinginfo, modeltype)
    #
...
#
def build_explmodel(
    model, explmodel, modvars, modcons, RSinginfo, CSinginfo, modeltype
):
    starttime = time.time()
    t1 = 0.0
    t2 = 0.0
    ...
    if modeltype == BYROWS:
        ...
        for con in model.getConstrs():
            ...
            explvardict[con.index] = explmodel.addVar(
                lb=-float("inf"), name=con.ConstrName
            )
        if _debug:
            t2 = time.time()
            sumtime += t2 - t1
        ...
        if CSinginfo != None:
            ...
            for con in modcons:  # slack basic variables
                ...
                explmodel.addVar(obj=0.0, lb=-float("inf"), name=vname, column=col)
                #
                # Slacks are column singletons by definition; treat them as
                # part of column singleton diagonal matrix even if also a row
                # singleton.
...
def kappa_stats(model, data, KappaExact):
    kappa = model.Kappa
    kappaexact = -1
    if KappaExact == 1 and (model.numConstrs < 1000000 and model.numVars < 1000000):
        kappaexact = model.KappaExact
    print("--------------------------------------------------------")
    print("Condition number stats for basis for model ", model.ModelName, ":")
    ...
    for k in range(quadexpr.size()):
        ...
        if xi.index in newvardict:  # xi is split into xi(+) - xi(-)
            ...
            if xj.index in newvardict:  # both xi and xj split
                ...
                # xi(-) * xj(-) term
                quadexpr.addTerms(qcoef, newvardict[xi.index], newvardict[xj.index])
...
def refine_output(model, absmultdict, expltype, submatrix):
    ...
    if expltype == BYROWS:
        ...
        #
        sorteddict = dict(sorted(absmultdict.items(), key=lambda x: x[1], reverse=True))
        #
        ...
        for varname in sorteddict.keys():
            ...
            model.addVar(
                lb=vartoadd.lb,
                ub=vartoadd.ub,
                column=model.getCol(vartoadd),
                name=vartoadd.VarName,
            )
...
def refine_col_output(model, abscolmultdict):
    ...
    # https://www.freecodecamp.org/news/sort-dictionary-by-value-in-python/
    #
    sorteddict = dict(sorted(abscolmultdict.items(), key=lambda x: x[1], reverse=True))
    #
    # Add the variables in sorted order to the end of the model, then
    # remove their duplicates in unsorted order that are the first varcnt
    # variables
